...
private:
...
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__TOS_WIN__)
	std::string m_sGNUPlotFileName = "gnuplot.exe";
	...
	// ---------------------------------------------------                          
	inline std::string    create_tmpfile(std::ofstream &tmp);
	...
	// ---------------------------------------------------------------------------------                                  
	inline bool file_available(const std::string &filename);
	...
	/// \return file exists (yes == true, no == false)
	// ---------------------------------------------------------------------------------                                  
	inline static bool    file_exists(const std::string &filename, int mode = 0);
...
public:
	...
	// ----------------------------------------------------------------------------
	inline bool set_GNUPlotPath(const std::string &path);
	...
	/// \return ---
	// ----------------------------------------------------------------------------
	inline void set_terminal_std(const std::string &type);
	...
	inline Gnuplot(const std::string &style = "points");

	/// plot a single std::vector at one go
	inline Gnuplot(const std::vector<double> &x,
	...
	inline Gnuplot& cmd(const std::string &cmdstr);
	// ---------------------------------------------------------------------------------
	...
	inline Gnuplot& showonscreen(); // window output is set by default (win/x11/aqua)

							 /// saves a gnuplot session to a postscript file, filename without extension
	inline Gnuplot& savetops(const std::string &filename = "gnuplot_output");
	...
	inline Gnuplot& set_style(const std::string &stylestr = "points");
	...
	inline Gnuplot& set_smooth(const std::string &stylestr = "csplines");
	...
	inline Gnuplot& set_pointsize(const double pointsize = 1.0);
	...
	inline Gnuplot& set_samples(const int samples = 100);
	/// set isoline density (grid) for plotting functions as surfaces (for 3d plots)
	inline Gnuplot& set_isosamples(const int isolines = 10);
	...
	///  base, surface, both
	inline Gnuplot& set_contour(const std::string &position = "base");
	// --------------------------------------------------------------------------
	...
	inline Gnuplot& set_legend(const std::string &position = "default");
	...
	/// set x axis label
	inline Gnuplot& set_ylabel(const std::string &label = "x");
	/// set y axis label
	inline Gnuplot& set_xlabel(const std::string &label = "y");
	/// set z axis label
	inline Gnuplot& set_zlabel(const std::string &label = "z");

	/// set axis - ranges
	inline Gnuplot& set_xrange(const double iFrom,
		const double iTo);
	/// set y-axis - ranges
	inline Gnuplot& set_yrange(const double iFrom,
		const double iTo);
	/// set z-axis - ranges
	inline Gnuplot& set_zrange(const double iFrom,
		const double iTo);
	...
	inline Gnuplot& set_cbrange(const double iFrom, const double iTo);
	...
	///   from file
	inline Gnuplot& plotfile_x(const std::string &filename,
		const unsigned int column = 1,
		const std::string &title = "");
	///   from std::vector
	template<typename X>
	inline Gnuplot& plot_x(const X& x, const std::string &title = "");


	/// plot x,y pairs: x y
	///   from file
	inline Gnuplot& plotfile_xy(const std::string &filename,
		const unsigned int column_x = 1,
		const unsigned int column_y = 2,
		const std::string &title = "");
	///   from data
	template<typename X, typename Y>
	inline Gnuplot& plot_xy(const X& x, const Y& y, const std::string &title = "");

	template<>
	inline Gnuplot& plot_xy(const double& x, const double& y, const std::string &title);


	/// plot x,y pairs with dy errorbars: x y dy
	///   from file
	inline Gnuplot& plotfile_xy_err(const std::string &filename,
		const unsigned int column_x = 1,
		const unsigned int column_y = 2,
	...
	inline Gnuplot& plot_xy_err(const X &x, const Y &y, const E &dy,
		const std::string &title = "");


	/// plot x,y,z triples: x y z
	///   from file
	inline Gnuplot& plotfile_xyz(const std::string &filename,
		const unsigned int column_x = 1,
		const unsigned int column_y = 2,
	...
	inline Gnuplot& plot_xyz(const X &x,
		const Y &y,
		const Z &z,
		const std::string &title = "");
	...
	inline Gnuplot& plot_slope(const double a,
		const double b,
		const std::string &title = "");
	...
	inline Gnuplot& plot_equation(const std::string &equation,
		const std::string &title = "");
	...
	inline Gnuplot& plot_equation3d(const std::string &equation,
		const std::string &title = "");


	/// plot image
	inline Gnuplot& plot_image(const unsigned char *ucPicBuf,
	...
	inline Gnuplot& reset_plot();

	/// resets a gnuplot session and sets all variables to default
	inline Gnuplot& reset_all();

	/// deletes temporary files
	inline void remove_tmpfiles();
...
template<>
Gnuplot& Gnuplot::plot_xy<>(const double& x, const double& y, const std::string &title)
{
	std::ofstream tmp;
	std::string name = create_tmpfile(tmp);
	if (name == "")
...
std::string Gnuplot::create_tmpfile(std::ofstream &tmp)
{
	...
	//
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__TOS_WIN__)
	if (_mktemp_s(name, strlen(name) + 1) != NULL)
#elif defined(unix) || defined(__unix) || defined(__unix__) || defined(__APPLE__)
	if (mkstemp(name) == -1)
#endif
	{
